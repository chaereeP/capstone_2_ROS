...
tt_core_msgs::XboxFlagPtr msg_ctrl_;
...
void dataSend(int _flag)
{
    msg_ctrl_->header.stamp = signal_time;
    msg_ctrl_->flag = _flag;
    msg_ctrl_->Float32MultiArray.data.clear();
    data_mutex_.lock();
    // std::cout << "Data[20]: " << data[20] << ", data[21]: " << data[21] << std::endl;
    //msg_ctrl_->Float32MultiArray.data;
	msg_ctrl_->Float32MultiArray.data.insert(msg_ctrl_->Float32MultiArray.data.end(), data, data+24);
    data_mutex_.unlock();
...
{
	int i,j,jLow, jHigh; //i,j represents theLost ball
...
int flag_keep_hole = 0;
int max_keep_hole = 5;

void callback_map(const tt_core_msgs::MapFlagConstPtr& msg){
    dataInit();
	int ballPosX,ballPosY;
	int targetPosX,targetPosY;
	bool findBall,findBarrier;

    signal_time = msg->header.stamp;

	cv_bridge::CvImageConstPtr cv_ptr;
	try
	{
        boost::shared_ptr<void const> tracked_object_tmp;
		cv_ptr = cv_bridge::toCvShare(msg->frame, tracked_object_tmp);
	}
    ...
    std::cout<<"Flag "<<msg->flag<<std::endl;
    ...
    {
        data[14] = 1;
        switch(msg->track_state)
        {
            case 0: //unknown target
                flag_keep_hole = max_keep_hole;
                break;
            case 1: //large target
                data[5] = -1;
                flag_keep_hole = max_keep_hole;
...
int main(int argc, char** argv)
{
	...
	ros::NodeHandle nh;
	msg_pub = nh.advertise<tt_core_msgs::XboxFlag>("/motion_planner/ctrl", 1);
	msg_ctrl_.reset(new tt_core_msgs::XboxFlag);
