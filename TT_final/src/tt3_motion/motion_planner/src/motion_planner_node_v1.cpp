...
void track_ball2(int x, int y, int targetPosX, int targetPosY,bool findBarrier);
...
void callback_map(const sensor_msgs::ImageConstPtr& msg){
	...
	if(flag_auto)
	{
		...
		{
            track_ball2(ballPosX,ballPosY, targetPosX,targetPosY, findBarrier);
...
void track_ball2(int ballPosX, int ballPosY, int targetPosX, int targetPosY,bool findBarrier)
{
    // 0: Above, far
    // 1: Close, eat
    // 2: Under the duct
    ...
    std::cout<<"Track State :"<<trackState<<std::endl;
    std::cout<<"dist Vertical above switch :"<<distVertical<<std::endl;
    switch(trackState){
        case 0: // Above, far, full speed operation
            if(distVertical != 0)
    		      data[8] = -rotationHSConstant*atan(distHorizontal/distVertical); //rotate
        ...
        case 1: // Close, Eat
            if(distVertical!=0){
                data[8] = -rotationLSConstant*atan(distHorizontal/distVertical);
                data[4] = ductDirectionConstant*atan(1.0*distHorizontal/distVertical);
            }
...
void track_ball(int ballPosX, int ballPosY, int targetPosX, int targetPosY,bool findBarrier)
{
	...
	distHorizontal = ballPosX - originH;
	distVertical = originV - ballPosY;

	distAbsolute = sqrt(distHorizontal*distHorizontal + distVertical*distVertical);

	if(distAbsolute >fullSpeedDist) //full speed operation
	{
		data[8] = rotationHSConstant*atan(distHorizontal/distVertical);
	...
	else // Non full-speed & eat
	{
		...
		{
			data[14] = 1;
			data[4] = 1.0*atan(1.0*distHorizontal/distVertical)*4/M_PI;
			eatCount = 0;
		}
		else
		{
