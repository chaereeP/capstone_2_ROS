...
Transition = collections.namedtuple("Transition", ["state", "action", "reward", "next_state", "done", "rnn_state"])
...
class Worker(object):
    ...
    def run(self, sess, coord, t_max):
        with sess.as_default(), sess.graph.as_default():
            # Initial state
            self.state = np.stack([self.env.reset().reshape(93,93)] * 4, axis=2)
            self.rnn_state = self.policy_net.state_init
            try:
    ...
    def _value_net_predict(self, state, rnn_state, sess):
        self.value_net.state_in[0] = rnn_state[0]
        self.value_net.state_in[1] = rnn_state[1]
    ...
    def run_n_steps(self, n, sess):
        ...
        for _ in range(n):
            # Take a step
            action_probs, next_rnn_state = self._policy_net_predict(self.state, self.rnn_state, sess)
            action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
            next_state, reward, done = self.env.step(action)
            next_state = np.append(self.state[:,:,1:], np.expand_dims(next_state.reshape(93,93), 2), axis=2)

            # Store transition
            transitions.append(Transition(state=self.state, action=action, reward=reward, next_state=next_state, done=done, rnn_state=self.rnn_state))
    ...
    def update(self, transitions, sess):
        ...
        if not transitions[-1].done:
            reward = self._value_net_predict(transitions[-1].next_state, transitions[-1].rnn_state, sess)
        ...
        for transition in transitions[::-1]:
            ...
            policy_target = (reward - self._value_net_predict(transition.state, transition.rnn_state, sess))
            # Accumulate updates
