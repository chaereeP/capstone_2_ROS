...
tf.flags.DEFINE_string("model_dir", "data/train", "Directory to write Tensorboard summaries and videos to.")
...
def make_env(wrap=True, thread_name=""):
    env = simulator.Task(debug_flag=False, test_flag=False, state_blink=True, state_inaccurate=True, thread_name=thread_name)
    return env
...
with tf.device("/cpu:0"):
    ...
    pe = PolicyMonitor(
        env=make_env(thread_name=str("p")),
        policy_net=policy_net,
