...
def build_shared_network(X, rnn_state_in, add_summaries=False):
    """
    ...
    with tf.variable_scope("lstm1"):
        ...
        rnn_state_in = tf.contrib.rnn.LSTMStateTuple(rnn_state_in[0], rnn_state_in[1])
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(LSTM_CELL_UNIT, state_is_tuple=True)
        lstm_outputs, lstm_state = tf.nn.dynamic_rnn(
            lstm_cell, rnn_in,
            initial_state=rnn_state_in,
            time_major=False)
        lstm_c, lstm_h = lstm_state
        rnn_state_out = (lstm_c[:1, :], lstm_h[:1, :])
        rnn_out = tf.reshape(lstm_outputs, [-1, LSTM_CELL_UNIT])
    ...
    return fc1, rnn_out, rnn_state_out
...
class PolicyEstimator():
    ...
    def __init__(self, num_outputs, reuse=False, trainable=True):
        ...
        self.states = tf.placeholder(shape=[None, 93, 93, 4], dtype=tf.uint8, name="X")
        # The TD target value
        ...
        c_init = np.zeros((1, LSTM_CELL_UNIT), np.float32)
        h_init = np.zeros((1, LSTM_CELL_UNIT), np.float32)
        self.state_init = [c_init, h_init]
        # c_in = tf.placeholder(shape=[1, LSTM_CELL_UNIT], dtype=tf.float32)
        # h_in = tf.placeholder(shape=[1, LSTM_CELL_UNIT], dtype=tf.float32)
        self.state_in = [c_init, h_init]
        self.state_out = [c_init, h_init]

        # Graph shared with Value Net
        with tf.variable_scope("shared", reuse=reuse):
            fc1, rnn_out, rnn_state_out = build_shared_network(X, self.state_in, add_summaries=(not reuse))

        with tf.variable_scope("policy_net"):
            self.state_out[0] = rnn_state_out[0]
            self.state_out[1] = rnn_state_out[1]

            # self.logits = tf.contrib.layers.fully_connected(rnn_out, num_outputs, activation_fn=None)
            self.logits = tf.contrib.slim.fully_connected(fc1, num_outputs,
                                               activation_fn=None,
...
class ValueEstimator():
    ...
    def __init__(self, reuse=False, trainable=True):
        ...
        h_init = np.zeros((1, LSTM_CELL_UNIT), np.float32)
        self.state_init = [c_init, h_init]
        # c_in = tf.placeholder(tf.float32, [1, LSTM_CELL_UNIT])
        # h_in = tf.placeholder(tf.float32, [1, LSTM_CELL_UNIT])
        self.state_in = [c_init, h_init]
        self.state_out = [c_init, h_init]
        ...
        with tf.variable_scope("value_net"):
            ...
            #     num_outputs=1,
            #     activation_fn=None)
            self.logits = tf.contrib.slim.fully_connected(fc1, 1,
                                               activation_fn=None,
