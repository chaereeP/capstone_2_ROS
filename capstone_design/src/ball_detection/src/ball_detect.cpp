...
int low_h_r_1=160, high_h_r_1=180, low_s_r_1=190, low_v_r_1=85;
...
int low_h_b=70, low_s_b=150, low_v_b=30;
...
int iMin_tracking_ball_size = 25;
double fontScale = 2;
int thickness = 3;
String text ;
    ...
    int count_r=0;
 	for( size_t i = 0; i< contours_r.size(); i++ ){
    	if((radius_r[i] > iMin_tracking_ball_size) && (15<center_r[i].x)&&(center_r[i].x < 605)){ //for erasing errors using min radius and side of the screen
            ...
            cout <<"red"<<i<< "\t"<<cx<< "\t"<<distance<<endl;
		}
    }
    		cout<<"blue"<<count_b<<endl;
		    cout<<"red"<<count_r<<endl;
...
/*
    ...
    for(int k=0;k<circles.size();k++){
        ...
        msg.img_y[k]=cy;
		msg.dist[k] = distance;
 		std::cout <<cx<<"\t"<<cy<<"\t"<< distance << std::endl;
...
void depthCallback(const sensor_msgs::ImageConstPtr& msg)
{
   ...
   try
   {
     buffer2 = cv_bridge::toCvShare(msg)->image;  //transfer the image data into buffer
   }
   catch (cv_bridge::Exception& e)
   {
     ROS_ERROR("Could not convert from '%s' to 'bgr8'.", msg->encoding.c_str());
   } //proceed ball detection
}
*/
void depthCallback(const sensor_msgs::ImageConstPtr& msg)
{
...
vector<float> pixel2point(Point center, int distance){vector<float> position;
    ...
    Zc = roundf(Yc * 1000) / 1000;
    Zc = sqrt(pow(Zc,2)-pow(car_height,2)-pow(Xc,2));
